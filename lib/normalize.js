...
var Normalizer = exports.Normalizer = (
  function() {
    ...
    function splitFile(filePath, callback) {
      var cmd = 'mp3splt -s -p th=-30,rm ' + filePath;
      sys.puts(cmd);
    ...
    function flacConversion(filePath, callback) {
      ...
      var convertedFlacFilePath = __dirname + '/' + convertedFlacFileName;
      var cmd = 'sox ' + filePath + ' ' + convertedFlacFileName + ' gain -n -5 silence 1 5 2%';
      sys.puts(cmd);
      var child = exec(cmd,
		       function (err, stdout, stderr) {
			 if (err) {
			   ...
			   return callback(null, convertedFlacFileName);
			 }
		       });
    }
